...
export function KpiCard({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "flex w-full items-center gap-2 rounded-lg border bg-card/60 px-2 py-1.5 text-left",
    ...
    >
      <span className={cn("flex h-7 w-7 shrink-0 items-center justify-center rounded-md", TONE[tone])}>
      ...
      {clickable ? <ChevronRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground" /> : null}
    </motion.button>
  );
}
