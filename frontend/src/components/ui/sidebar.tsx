...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  // This is the internal state of the sidebar. Restore from cookie so state persists across navigation.
  const [_open, _setOpen] = React.useState(() => {
    const fromCookie = getSidebarStateFromCookie();
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          className="flex h-full w-[--sidebar-width] flex-col border-sidebar-border bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
          style={
            {
        ...
        >
          ...
          <SheetDescription className="sr-only">Switch pages and manage your account.</SheetDescription>
          <div className="flex h-full min-h-0 w-full flex-col overflow-hidden pb-[env(safe-area-inset-bottom)]">
            {children}
          </div>
        </SheetContent>
      </Sheet>
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-svh w-[--sidebar-width] bg-transparent transition-[width] duration-300 ease-expo",
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-[--sidebar-width] overflow-hidden transition-[left,right,width] duration-300 ease-expo lg:flex",
          side === "left"
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden sidebar-scroll",
        className,
      )}
      {...props}
    />
...
const SidebarGroup = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col p-2 group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:px-1.5", className)}
      {...props}
    />
  );
});
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button relative flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-md px-2.5 py-1.5 text-left text-[13px] font-medium outline-none ring-sidebar-ring transition-[width,height,padding] duration-300 ease-expo hover:bg-sidebar-accent/70 hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-transparent data-[active=true]:font-medium data-[active=true]:text-white data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!mx-auto group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!justify-center group-data-[collapsible=icon]:!p-0 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
